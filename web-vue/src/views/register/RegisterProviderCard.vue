    ...
    <div class="register-provider-section">
      ...
      <div class="register-form-grid register-form-grid--two">
        ...
        <label v-if="currentType === 'icloud_api' || currentType === 'icloud_local'" class="register-field">
          ...
          <Input
            :model-value="provider.project"
            block
            :disabled="disabled"
            placeholder="openai"
            @update:model-value="value => emit('update-field', index, 'project', String(value || '').trim())"
          />
        </label>

        <label v-if="currentType === 'icloud_api' || currentType === 'icloud_local'" class="register-field">
          <span class="register-label">用途</span>
          <Input
        ...
        <label v-if="currentType === 'icloud_api' || currentType === 'icloud_local'" class="register-field">
          <span class="register-label">邮件关键词</span>
          <Input
            :model-value="provider.keyword"
            block
            :disabled="disabled"
            placeholder="OpenAI"
            @update:model-value="value => emit('update-field', index, 'keyword', String(value || '').trim())"
          />
        </label>

        <label v-if="currentType === 'icloud_api' || currentType === 'icloud_local'" class="register-field">
          <span class="register-label">长轮询等待时间 (ms)</span>
          <Input
            :model-value="provider.wait_ms"
            type="number"
            min="0"
            max="30000"
            step="1000"
            block
            :disabled="disabled"
            @update:model-value="value => emit('update-field', index, 'wait_ms', numberModelValue(value))"
          />
        </label>

        <label v-if="currentType === 'icloud_api'" class="register-checkbox-field register-checkbox-field--compact register-field--full">
          <Checkbox
