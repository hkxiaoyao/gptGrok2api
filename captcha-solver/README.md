...
> GPTGrok2API bundled component. The source was imported from
> [Xesoxeee/xai-grok-mass](https://github.com/Xesoxeee/xai-grok-mass/tree/main/captcha-solver)
> at commit `755e4b8`, then extended with per-request Turnstile proxy forwarding
> and the xAI real-page widget/callback token fixes. It now ships directly in
> the main repository at `captcha-solver/`; no second repository is required.
