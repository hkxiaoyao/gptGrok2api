...
systemd 主服务启动后会自动运行 Grok 账号探测，不依赖浏览器页面，也不需要手动启用。默认每 `60` 分钟执行一轮，每批 `50` 个账号；首次没有历史记录时立即执行。

调度器只处理已加入 Grok 运行池、保存 SSO 且未禁用的账号。它使用 Fast 配额探针，不发送 Console 对话；明确失效时使用已保存邮箱和密码重新登录，新 SSO 验证成功后再替换旧 SSO。未知状态不会触发登录，失败按指数退避重试，最长 `24` 小时。

## 17. 并发和多 worker

Captcha Solver 使用共享动态并发限制器，不需要为了 Turnstile 并发启动多个 Uvicorn worker。注册中心的“注册解题并发”是注册任务和 Grok OAuth 共享的浏览器并发上限；程序不会暗中增加额外槽位。

推荐步骤：

1. 先用注册并发 `1`。
2. 连续成功后改为 `2`。
3. 16 GB 以上服务器可尝试注册线程 `3`、注册解题并发 `3`；需要同时承载更多注册和 OAuth 解题时再主动提高 solver 并发。

注意：

- 每个活跃槽位都可能启动独立 Chromium。
- 内存紧张时把注册线程和注册解题并发同时降为 `2`，OAuth 会与注册任务共享这两个槽位。
