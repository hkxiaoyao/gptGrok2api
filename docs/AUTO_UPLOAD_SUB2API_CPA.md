...
- Grok 每个账号注册成功后会立即进入 2-worker OAuth 优先级队列，不需要等待整批注册结束；新注册任务优先于未授权回填和失败重试，NovaApi 和 CPA 会在该账号 OAuth 成功后按配置投递。
...
OAuth 队列固定为 `2` 个 worker，并通过账号任务复用避免同一账号重复授权。Captcha Solver 不再暗中增加额外槽位，“注册解题并发”就是注册和 OAuth 共享的浏览器并发上限；需要让两路 OAuth 同时解题时，应结合注册并发和内存主动提高该值。
